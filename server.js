...
var getImage = edge.func(function () {/*
 ...
 middle = message.IndexOf(" ", middle);

 string messageOne = middle >= 0 ? message.Substring(0, middle) : message;
 string messageTwo = middle >= 0 ? message.Substring(middle) : string.Empty;

 Bitmap image = (Bitmap)Image.FromFile("giorgio-tsoukalos.jpeg");
 Graphics graphics = Graphics.FromImage(image);

 Font fontTwo = (Font)null;
 Font fontOne = (Font)null;

 SizeF fontMeasureOne = new SizeF(1000, 1000);
 SizeF fontMeasureTwo = new SizeF(1000, 1000);

 for (int fontSize = 80; fontSize > 0; fontSize = fontSize - 5)
 {
 if ((ushort)fontMeasureOne.Width > (ushort)image.Width)
 {
 fontOne = new Font("arial", fontSize, FontStyle.Bold);
 fontMeasureOne = graphics.MeasureString(messageOne, fontOne);
 }
 if ((ushort)fontMeasureTwo.Width > (ushort)image.Width)
 {
 fontTwo = new Font("arial", fontSize, FontStyle.Bold);
 fontMeasureTwo = graphics.MeasureString(messageTwo, fontTwo);
 }
 }

 int top = 5;
 int left = (int)((image.Width - fontMeasureOne.Width) / 2);

 graphics.DrawString(messageOne, fontOne, Brushes.White, left, top);

 top = image.Height - (int)fontMeasureTwo.Height - 5;
 left = (int)((image.Width - fontMeasureTwo.Width) / 2);
...
function loadIndex(reqUrl, req, res) {
    fs.readFile('index.html', function(err, data) {
        if (err) error500(req, res);
...
function loadImage(reqUrl, req, res){
    var index = 0;

    if (reqUrl.query &&
        reqUrl.query.index &&
        !isNaN(reqUrl.query.index))
    {
        index = reqUrl.query.index;
    }
    else
    {
        index = Math.floor((Math.random()*465)+1);
    }


    var msg = quotes[index];

    getImage(msg, function (error, result) {
...
http.createServer(function(req, res) {
    ...
    if(routes[reqUrl.pathname]){
        routes[reqUrl.pathname](reqUrl, req, res);
    }
    else{
        error404(req, res);
    }
